...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const doc = await db.collection("team_members").findOne({ _id });
...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (body.socials !== undefined) allowed.socials = body.socials;
    ...
    const db = client.db();
    const _id = new ObjectId(params.id);
    const res = await db.collection("team_members").findOneAndUpdate(
      { _id },
      { $set: allowed },
...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const db = client.db();
    const _id = new ObjectId(params.id);
    const res = await db.collection("team_members").deleteOne({ _id });
