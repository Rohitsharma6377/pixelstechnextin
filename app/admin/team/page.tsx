...
import { createTeam, deleteTeam, fetchTeam } from "@/features/admin/adminSlice";
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-end">
        <button onClick={() => setOpenCreate(true)} className="rounded bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-indigo-500">New Member</button>
      </div>

      <Modal open={openCreate} onClose={() => setOpenCreate(false)} title="New Team Member">
        <form onSubmit={onCreate} className="grid gap-4 md:grid-cols-2">
          ...
          <div className="md:col-span-2 flex justify-end gap-2">
            ...
            <button disabled={loading} className="rounded bg-indigo-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{loading ? "Saving..." : "Add member"}</button>
          </div>
        </form>
      </Modal>
            ...
            {team.map((m) => (
              <tr key={m._id}>
                ...
                <td className="px-4 py-3 text-sm">
                  <button onClick={() => dispatch(deleteTeam(m._id)).then(() => dispatch(fetchTeam()))} className="text-red-600">Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
